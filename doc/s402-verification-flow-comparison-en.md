# S402 Payment Verification Flow Comparison

This document compares two execution flows for S402 payment verification: synchronous verification and asynchronous verification.
...
```mermaid
flowchart TD
    A[Client Request] --> B{S402 Proof Provided?}
    B -->|No| C[Return 402 Payment Required]
    B -->|Yes| D[Validate Proof Format]
    D --> E[Parameter Validation<br/>Amount/Recipient/Deadline]
    E --> F[EIP-712 Signature Verification]
    F --> G[Blockchain Transaction Verification<br/>Wait for Confirmation]
    G --> H{Verification Successful?}
    H -->|Yes| I[API Access Granted]
...
flowchart TD
    ...
    D --> E[Parameter Validation<br/>Amount/Recipient/Deadline]
    E --> F[EIP-712 Signature Verification]
    F --> G[Immediately Grant API Access]
    G --> H[Start Background Blockchain Verification]
    H --> I{Verification Result}
    I -->|Success| J[Log Success]
    I -->|Failure| K[Log Warning]
    G --> L[Execute Controller Logic]
```
