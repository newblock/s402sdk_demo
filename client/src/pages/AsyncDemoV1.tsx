...
export default function AsyncDemo() {
  ...
  const sendPayment = async () => {
    try {
      ...
      // For async mode: send transaction and immediately call API without waiting for confirmation
      if (asyncMode) {
        toast({ title: "Sending Payment", description: "Submitting transaction..." });
        ...
        // Set tx hash immediately for user feedback
        setTxHash(tx.hash);
        
        // Build S402 proof for API call
        const s402Proof = {
          payment,
          authSig: authSigStruct,
          txHash: tx.hash
        };
        
        // Call async API immediately without waiting for transaction confirmation
        toast({ title: "Testing Async API", description: "Calling API with payment proof..." });
        
        const response = await fetch('/api/v1/tool/example-async', {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            payload: { test: "async payment demo" },
            s402Proof
          }),
        });
        ...
        // Sync mode: wait for transaction confirmation before calling API
